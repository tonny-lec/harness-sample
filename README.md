...
- コマンド・テスト・検証の結果を確認した直後に、状態ファイルの内容全体を現時点の 4 項目(目的・状態・決定・次の一手)の箇条書きに書き換え、その時点の推論(計画・仮説と検証結果・発見・判断とその理由・確定した事実と知識)をノートへ追記する
...
担当タスクの状態ファイルとノート、次いで記憶(`docs/memory/`)を使い、生ログは取りこぼしを調べる最終手段とします。
