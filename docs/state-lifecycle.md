...
Codex は、コマンド・テスト・検証を実行して結果を確認した直後に、状態ファイルの
内容全体を現時点の 4 項目の箇条書きに書き換え、その時点の推論を担当タスクのノートへ追記します。記録するのは
...
ツール実行が続いたとき、ファイルの確認・状態の書き換えと数行の追記・作業再開を促す短い手順を
...
| `pre_compact.py` | 生ログ退避 | 推論の生成、compaction の停止、状態ファイル・ノートの読み書き |
| `post_tool_use.py` | ファイル未更新時のルール想起の注入 | 状態ファイル・ノートの読み書き、推論の生成 |
